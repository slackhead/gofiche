...
Repository: https://github.com/slackhead/gofiche/
Live example: http://r0t.uk 9999

-------------------------------------------------------------------------------

usage: fiche [-DepsdolBug].
             [-D] [-e] [-d domain] [-p port] [-s slug size]
             [-o output directory] [-B buffer size] [-u user name]
             [-l log file] [-g gopher domain]
 ...
 */
static int create_directory(char *output_dir);
...
static void *handle_connection(void *args) {
    ...
    while(create_directory(c->settings->output_dir_path));
    ...
    // Write a response to the user
    {
        // Create an url (additional byte for slash and one for new line)
				size_t len = strlen(c->settings->domain) + strlen(slug) + 3;

				if (c->settings->godomain != NULL)
					len = strlen(c->settings->godomain) + strlen(slug) + 1 + strlen(c->settings->domain) + strlen(slug) + 4;

        char url[len];
				if (c->settings->godomain != NULL)
					snprintf(url, len, "%s%s%s%s%s%s%s%s", c->settings->godomain, "/", slug, "\n", c->settings->domain, "/", slug, "\n");
				else
...
static int save_to_file(const Fiche_Settings *s, uint8_t *data, char *slug) {
    ...
    size_t len = strlen(s->output_dir_path) + strlen(slug) + 2;
    ...
    snprintf(path, len, "%s%s%s", s->output_dir_path, "/", slug);
